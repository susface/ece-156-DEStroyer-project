/*
 *  thrust/system/detail/errno.h -- local compatibility shim
 *
 *  MSVC 19.45+ (VS 2022 v17.5+) injects a top-level `detail` namespace from
 *  STL internals. The original CCCL 2.x header uses C++17 nested-namespace
 *  syntax (`namespace system::detail { }`) which, when expanded inside
 *  THRUST_NAMESPACE_BEGIN, correctly produces `thrust::system::detail`.
 *  However, `error_code.h` then references these as bare `detail::eXXX`
 *  inside `thrust::system` — and nvcc's EDG frontend resolves that lookup
 *  to both `thrust::system::detail` AND the MSVC-injected top-level `detail`,
 *  triggering "thrust::detail is ambiguous".
 *
 *  This shim uses explicit nested `namespace system { namespace detail { } }`
 *  syntax, which is unambiguous in all nvcc/EDG versions.
 *
 *  Values are identical to the originals in CUDA 13.2 CCCL.
 */
#pragma once
